...
I will exercise binary grading, where assignments that (1) meet requirements and are (2) submitted on time are scored, and otherwise a zero is earned on the assignment. Across the binary graded assignments, each will be evening weighted across all lectures (tentatively 7 in person and 6 remote). Completion will be translated to letter grades on what I consider a ``typical"\footnote{\url{https://en.wikipedia.org/wiki/Academic_grading_in_the_United_States}} grading scale.
...
\item \href{https://pythoninchemistry.org/intro_python_chemists/intro.html}{An Introduction to Python for Chemistry}
\end{itemize}
\end{itemize}
